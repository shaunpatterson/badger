...
func (it *Iterator) parseItem() bool {
	...
	// Hide keys covered by a range tombstone at a newer version <= readTs.
	if !it.opt.InternalAccess &&
		it.txn.db.coveredByRangeTombstone(y.ParseKey(key), version, it.readTs) {
		mi.Next()
		return false
	}
